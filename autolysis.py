...
# Extract only the name of the .csv file without extension
...
# Utility function for making API calls with retry mechanism
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10))
def make_api_call(prompt):
    """
    Makes a POST request to the AI Proxy API with retry logic.

    Args:
        prompt (dict): The payload to be sent to the API.

    Returns:
        dict: The JSON response from the API.

    Raises:
        HTTPError: If the request fails after the maximum retry attempts.
    """
...
def execute_generated_code(prompt, data):
    """
    Executes AI-generated Python code based on the provided prompt and data.

    Args:
        prompt (dict): The input prompt sent to the AI model, structured as a dictionary.
        data (Any): The data passed as an argument to the generated function.

    Returns:
        Any: The output generated by the executed function.

    Raises:
        Exception: If the generated code produces an error during execution.
    """
    # Call the AI proxy to generate code
    response = call_ai_proxy(prompt)

    # Extract the function code from the AI response
    function_code = response['choices'][0]['message']['content']

    # Clean the generated code:
    # 1. Remove 'python' keyword if present.
    # 2. Remove all 'import' lines.
    # 3. Extract only the function definition and core logic.
    ...
    # Execute the generated function and run the analysis
    try:
        exec(function_code, globals())  # Execute the cleaned function code
        analysis_output = LLM_generated_code(data)  # Call the generated function
    except Exception as error:
        # Modify the prompt to request corrections for the faulty code
        prompt['message'][0]['content'] = 'You are a professional Python programmer.'
        prompt['message'][1]['content'] = f"Correct the following code: {function_code}\nError encountered: {error}"
...
def perform_advanced_analysis(data: pd.DataFrame):
    ...
    Returns:
        dict: The result of the analysis performed by the selected functions.
    """

    # Define the available analytical functions
    functions = [
        {
            "type": "function",
            "function": {
                "name": "Regression_analysis",
                "description": "Performs regression to analyze relationships between a dependent variable and independent variables.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "dataset": {"type": "object", "description": "Dataset used for analysis."},
                        "dependent_variable": {"type": "string", "description": "Column for the dependent variable."},
                        "independent_variables": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": "Columns for independent variables."
                        }
                    },
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "Analyzes trends, moving averages, and seasonality in time series data.",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "dataset": {"type": "object", "description": "Dataset used for analysis."},
                        "time_column": {"type": "string", "description": "Column with time values."},
                        "column_to_analyze": {"type": "string", "description": "Numeric column to analyze."}
                    },
                    "required": ["dataset", "time_column", "column_to_analyze"],
                    "additionalProperties": False
                }
            }
        },
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "columns_to_cluster": {
                            "type": "string",
                            "description": "Columns used as features for clustering, with transformations (e.g., 'Onehot_encoding', 'Ordinal_encoding')."
                        },
                        "number_of_clusters": {
                            "type": "integer",
    ...
    def run_function(prompt, retries=0):
        """
        Executes the API call and handles retries for faulty responses.

        Args:
            prompt (dict): The API request payload.
            retries (int): Current retry count.

        Returns:
            dict: Combined output from all successfully executed functions.
        """
        response = call_ai_proxy(prompt)
        ...
        for tool_call in tool_calls:
            ...
            function_args['dataset'] = data  # Add the dataset to function arguments
            ...
            try:
                ...
                print(f"Error in executing '{function_name}': {e}")
                
                # Reframe the prompt to handle the error and retry
                prompt['messages'][0]['content'] = f"You are a Professional Data Analyst. Correct the error: {e}."
                ...
                if retries < 3:
                    ...
                    print(f"Unable to resolve error in '{function_name}' after 3 retries.")

        return output

    # Generate the API request payload
    system_command = "You are a Professional Data Analyst. Perform meaningful analysis only."
    user_command = f"Provide JSON of feasible functions and columns for analysis. Sample: {convert_dates_to_iso(data.head(3).to_dict())}"
...
# Use LLM to generate asummary of the whole analysis
